...
function Onboarding({onComplete,onBack}){
  const [step,setStep]=useState(1);const [iv,setIv]=useState("");const [lc,setLc]=useState([]);const [jodpResult,setJodpResult]=useState(null);
  ...
  useEffect(()=>{if(step!==2)return;setLc([]);[0,1,2,3].forEach(i=>{setTimeout(()=>setLc(p=>[...p,i]),600+i*700);});let api=false,timer=false;const go=()=>{if(api&&timer)setStep(3);};setTimeout(()=>{timer=true;go();},600+4*700+600);sb.functions.invoke("fetch-jodp",{body:{registration_number:iv}}).then(({data})=>{setJodpResult(data);api=true;go();}).catch(()=>{api=true;go();});},[step]);
