...
function Onboarding({onComplete,onBack}){
  ...
  if(step===2){const items=["AJPES poslovni register","JODP de minimis evidenca","VIES davčna validacija","SKD → človeško berljive dejavnosti"];const subs=["Firma, SKD, naslov","Prejete pomoči, zneski","Aktiven zavezanec","Mapiranje oznak"];return(<div style={{minHeight:"100vh",background:c.ivory,fontFamily:f,display:"flex",alignItems:"center",justifyContent:"center",padding:isMobile?"24px 16px":24}}><div style={{width:"100%",maxWidth:440,textAlign:"center"}}><Loader2 size={32} color={c.olive} strokeWidth={2} style={{animation:"spin 1.2s linear infinite",marginBottom:24}}/><h2 style={{fontSize:22,fontWeight:700,color:c.t1,marginBottom:32}}>Pridobivam podatke …</h2><div style={{textAlign:"left",display:"flex",flexDirection:"column",gap:14}}>{items.map((item,i)=>{const done=lc.includes(i);return(<div key={i} style={{display:"flex",alignItems:"center",gap:12,opacity:done?1:0.3,transition:"opacity .4s",padding:"12px 16px",borderRadius:12,background:done?c.oliveLight:"transparent"}}><div style={{width:28,height:28,borderRadius:"50%",background:done?c.olive:`${c.t3}30`,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>{done?<Check size={14} color={c.white} strokeWidth={2.5}/>:<span style={{width:8,height:8,borderRadius:"50%",background:c.t3,opacity:.5}}/>}</div><div style={{flex:1,minWidth:0}}><div style={{fontSize:14,fontWeight:600,color:c.t1}}>{item}</div><div style={{fontSize:12,color:c.t2}}>{subs[i]}</div></div>{done&&<span style={{fontSize:11,fontWeight:600,color:c.olive,background:c.oliveMed,padding:"2px 10px",borderRadius:5}}>OK</span>}</div>);})}</div></div><style>{`@keyframes spin{to{transform:rotate(360deg)}}`}</style></div>);}
