...
function Nav({page,go,onStart}){
  return(
    <nav style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"18px 40px",background:`${c.white}cc`,backdropFilter:"blur(12px)",borderBottom:`1px solid ${c.border}50`,position:"sticky",top:0,zIndex:50,fontFamily:f}}>
      ...
      <div style={{display:"flex",alignItems:"center",gap:32}}>
        {[["kako","Kako deluje"],["cenik","Cenik"]].map(([k,l])=>(
          <a key={k} onClick={()=>go(k)} style={{fontSize:14,color:page===k?c.olive:c.t2,fontWeight:page===k?600:500,cursor:"pointer",textDecoration:"none",borderBottom:page===k?`2px solid ${c.olive}`:"2px solid transparent",paddingBottom:2}}>{l}</a>
        ))}
        <button onClick={onStart} style={{padding:"10px 24px",borderRadius:10,border:"none",background:c.graphite,color:c.white,fontSize:14,fontWeight:600,cursor:"pointer",fontFamily:f}}>Začni brezplačno</button>
      </div>
    </nav>
  );
}
...
function Footer(){
  return(
    <footer style={{borderTop:`1px solid ${c.border}`,padding:"32px 40px",display:"flex",justifyContent:"space-between",alignItems:"center",fontFamily:f}}>
...
function Landing({go,onStart}){
  const sec={maxWidth:1080,margin:"0 auto",padding:"0 32px"};
  return(
    <div style={{fontFamily:f,color:c.t1,background:c.ivory}}>
      <Nav page="landing" go={go} onStart={onStart}/>
      <section style={{...sec,padding:"100px 32px 80px",textAlign:"center"}}>
        ...
        <h1 style={{fontSize:52,fontWeight:700,lineHeight:1.08,color:c.t1,marginBottom:16,maxWidth:700,margin:"0 auto 16px"}}>AI prevod birokratskega jezika. <span style={{color:c.olive}}>Prave priložnosti.</span></h1>
        <p style={{fontSize:18,color:c.t2,lineHeight:1.6,maxWidth:540,margin:"0 auto 36px"}}>Vpišite matično številko. Sistem pridobi podatke, preveri de minimis stanje in predlaga razpise, ki ustrezajo vašemu podjetju.</p>
        <div style={{display:"flex",gap:12,justifyContent:"center"}}>
          <button onClick={onStart} style={{display:"flex",alignItems:"center",gap:10,padding:"16px 36px",borderRadius:14,border:"none",background:c.olive,color:c.white,fontSize:16,fontWeight:700,cursor:"pointer",fontFamily:f,boxShadow:`0 4px 24px ${c.olive}35`}}>Preveri razpise <ArrowRight size={20}/></button>
          <button onClick={()=>go("kako")} style={{display:"flex",alignItems:"center",gap:8,padding:"16px 28px",borderRadius:14,border:`1.5px solid ${c.border}`,background:c.white,color:c.t1,fontSize:16,fontWeight:600,cursor:"pointer",fontFamily:f}}>Kako deluje</button>
        </div>
        <div style={{display:"flex",justifyContent:"center",gap:40,marginTop:48,opacity:.5}}>{["148 razpisov","95 % natančnost matchinga","Pod 2 min onboarding"].map(t=><span key={t} style={{fontSize:13,fontWeight:500,color:c.t2}}>{t}</span>)}</div>
      </section>

      {/* Before/After teaser */}
      <section style={{...sec,padding:"60px 32px 80px"}}>
        <div style={{display:"flex",gap:20,maxWidth:800,margin:"0 auto"}}>
          ...
          <div style={{display:"flex",alignItems:"center"}}><ArrowRight size={24} color={c.olive}/></div>
      ...
      {/* Bottom CTA */}
      <section style={{...sec,padding:"80px 32px",textAlign:"center"}}>
...
function HowItWorks({go,onStart}){
  const sec={maxWidth:900,margin:"0 auto",padding:"0 32px"};
  const card={background:c.white,border:`1px solid ${c.border}`,borderRadius:16,padding:"28px 26px",marginBottom:16};
  return(
    <div style={{fontFamily:f,color:c.t1,background:c.ivory}}>
      <Nav page="kako" go={go} onStart={onStart}/>

      {/* Hero */}
      <section style={{...sec,padding:"72px 32px 56px",textAlign:"center"}}>
        <h1 style={{fontSize:42,fontWeight:700,lineHeight:1.1,color:c.t1,marginBottom:12}}>Kako deluje</h1>
      ...
      {/* 3 koraki */}
      <section style={{...sec,padding:"0 32px 64px"}}>
        {[
          ...
          <div key={step.num} style={{...card,padding:"36px 32px",marginBottom:24}}>
      ...
      {/* Matching osi */}
      <section style={{...sec,padding:"64px 32px"}}>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:10}}>
          {[
            ...
            <div key={a.n} style={{display:"flex",alignItems:"center",gap:16,padding:"18px 20px",borderRadius:14,background:c.white,border:`1px solid ${c.border}`}}>
              ...
              <div style={{textAlign:"right",flexShrink:0}}><div style={{fontSize:20,fontWeight:800,color:c.olive}}>{a.w}</div><div style={{fontSize:11,color:c.t3}}>utež</div></div>
            </div>
          ))}
        </div>
      </section>
...
function Pricing({go,onStart}){
  ...
  return(
    <div style={{fontFamily:f,color:c.t1,background:c.ivory}}>
      ...
      {/* Hero */}
      <section style={{...sec,padding:"72px 32px 16px",textAlign:"center"}}>
        <h1 style={{fontSize:42,fontWeight:700,lineHeight:1.1,color:c.t1,marginBottom:12}}>Cenik</h1>
        <p style={{fontSize:17,color:c.t2,maxWidth:480,margin:"0 auto"}}>Brez vezave. Brez skritih stroškov. Prekličete kadarkoli.</p>
      </section>

      {/* Cards */}
      <section style={{...sec,padding:"40px 32px 56px"}}>
      ...
      <section style={{background:c.white,padding:"64px 0"}}>
        <div style={sec}>
          ...
          <div style={{borderRadius:16,border:`1px solid ${c.border}`,overflow:"hidden"}}>
            {/* Header row */}
            <div style={{display:"grid",gridTemplateColumns:"1fr repeat(4,140px)",background:c.ivory,borderBottom:`1px solid ${c.border}`}}>
            ...
            {compareFeatures.map((row,ri)=>{
              ...
              return(
                <div key={ri} style={{display:"grid",gridTemplateColumns:"1fr repeat(4,140px)",borderBottom:`1px solid ${c.border}20`}}>
      ...
      {/* FAQ */}
      <section style={{...sec,padding:"64px 32px 80px"}}>
...
function Onboarding({onComplete,onBack}){
  const [step,setStep]=useState(1);const [iv,setIv]=useState("");const [lc,setLc]=useState([]);
  ...
  useEffect(()=>{if(step!==2)return;setLc([]);[0,1,2,3].forEach(i=>{setTimeout(()=>setLc(p=>[...p,i]),600+i*700);});setTimeout(()=>setStep(3),600+4*700+600);},[step]);
  ...
  const mc={name:"Mizarstvo Novak d.o.o.",maticna:"1234567000",address:"Industrijska cesta 5, 2000 Maribor",region:"Vzhodna Slovenija",nuts:"SI03",legalForm:"d.o.o.",founded:2011,age:15,skdMain:"C16.230",skdMainLabel:"Stavbno mizarstvo in tesarstvo",skdOther:["C31.090","G46.730"],dm:{received:42000,limit:300000,free:258000},dmH:[{y:2024,s:"MGRT – Digitalni vavčer",a:9500},{y:2025,s:"SPIRIT – Vavčer za internacionalizacijo",a:14500},{y:2025,s:"Zavod RS za zaposlovanje",a:18000}]};

  if(step===1) return(<div style={{minHeight:"100vh",background:c.ivory,fontFamily:f,display:"flex",alignItems:"center",justifyContent:"center",padding:24}}><div style={{width:"100%",maxWidth:480,textAlign:"center"}}><button onClick={onBack} style={{position:"absolute",top:20,left:20,background:"none",border:"none",cursor:"pointer",display:"flex",alignItems:"center",gap:6,fontSize:13,color:c.t2,fontFamily:f}}><ChevronLeft size={18}/>Nazaj</button><div style={{display:"flex",alignItems:"center",justifyContent:"center",gap:10,marginBottom:32}}><div style={{width:44,height:44,borderRadius:12,background:c.olive,display:"flex",alignItems:"center",justifyContent:"center",fontWeight:800,fontSize:16,color:c.white}}>AI</div><span style={{fontSize:20,fontWeight:700,color:c.t1}}>RAZPISI</span></div><h1 style={{fontSize:28,fontWeight:700,lineHeight:1.15,color:c.t1,marginBottom:8}}>Vpišite matično ali davčno<br/>številko podjetja</h1><p style={{fontSize:15,color:c.t2,marginBottom:32}}>Sistem avtomatsko pridobi podatke iz AJPES registra, de minimis evidenco in preveri davčni status.</p><div style={{display:"flex",gap:10}}><input ref={ir} value={iv} onChange={e=>setIv(e.target.value.replace(/\D/g,""))} placeholder="npr. 1234567000" style={{flex:1,height:56,border:`2px solid ${iv.length>=8?c.olive:c.border}`,borderRadius:14,padding:"0 20px",fontSize:17,fontFamily:"monospace",color:c.t1,background:c.white,outline:"none",letterSpacing:".04em"}} onKeyDown={e=>{if(e.key==="Enter"&&iv.length>=8)setStep(2);}}/><button onClick={()=>{if(iv.length>=8)setStep(2);}} style={{...btn,width:"auto",padding:"0 28px",height:56,background:iv.length>=8?c.graphite:`${c.graphite}30`}}>Naprej <ArrowRight size={18}/></button></div></div></div>);

  if(step===2){const items=["AJPES poslovni register","JODP de minimis evidenca","VIES davčna validacija","SKD → človeško berljive dejavnosti"];const subs=["Firma, SKD, naslov","Prejete pomoči, zneski","Aktiven zavezanec","Mapiranje oznak"];return(<div style={{minHeight:"100vh",background:c.ivory,fontFamily:f,display:"flex",alignItems:"center",justifyContent:"center",padding:24}}><div style={{width:"100%",maxWidth:440,textAlign:"center"}}><Loader2 size={32} color={c.olive} strokeWidth={2} style={{animation:"spin 1.2s linear infinite",marginBottom:24}}/><h2 style={{fontSize:22,fontWeight:700,color:c.t1,marginBottom:32}}>Pridobivam podatke …</h2><div style={{textAlign:"left",display:"flex",flexDirection:"column",gap:14}}>{items.map((item,i)=>{const done=lc.includes(i);return(<div key={i} style={{display:"flex",alignItems:"center",gap:12,opacity:done?1:0.3,transition:"opacity .4s",padding:"12px 16px",borderRadius:12,background:done?c.oliveLight:"transparent"}}><div style={{width:28,height:28,borderRadius:"50%",background:done?c.olive:`${c.t3}30`,display:"flex",alignItems:"center",justifyContent:"center"}}>{done?<Check size={14} color={c.white} strokeWidth={2.5}/>:<span style={{width:8,height:8,borderRadius:"50%",background:c.t3,opacity:.5}}/>}</div><div style={{flex:1}}><div style={{fontSize:14,fontWeight:600,color:c.t1}}>{item}</div><div style={{fontSize:12,color:c.t2}}>{subs[i]}</div></div>{done&&<span style={{fontSize:11,fontWeight:600,color:c.olive,background:c.oliveMed,padding:"2px 10px",borderRadius:5}}>OK</span>}</div>);})}</div></div><style>{`@keyframes spin{to{transform:rotate(360deg)}}`}</style></div>);}

  return(<div style={{background:c.ivory,minHeight:"100vh",fontFamily:f}}><div style={{borderBottom:`1px solid ${c.border}`,background:c.white,padding:"16px 24px",display:"flex",alignItems:"center",gap:12,position:"sticky",top:0,zIndex:10}}><button onClick={()=>setStep(s=>Math.max(s-1,1))} style={{background:"none",border:"none",cursor:"pointer",padding:4}}><ChevronLeft size={20} color={c.t2}/></button><div style={{flex:1}}><div style={{fontSize:13,color:c.t2}}>Korak {step-2} od 4</div><div style={{fontSize:15,fontWeight:600,color:c.t1}}>{step===3?"Profil podjetja":step===4?"KMU klasifikacija":step===5?"Strateški interesi":"Rezultati"}</div></div><div style={{display:"flex",gap:6}}>{[3,4,5,6].map(s=><div key={s} style={{width:s===step?24:8,height:8,borderRadius:4,background:s<=step?c.olive:`${c.t3}30`}}/>)}</div></div><div style={{maxWidth:540,margin:"0 auto",padding:"28px 24px 60px"}}>
    {step===3&&<><h2 style={{fontSize:22,fontWeight:700,color:c.t1,marginBottom:24}}>Profil podjetja</h2><div style={{background:c.white,border:`1px solid ${c.border}`,borderRadius:16,padding:"20px 22px",marginBottom:16}}>{[["Firma",mc.name],["Matična",mc.maticna],["Naslov",mc.address],["Regija",`${mc.region} (${mc.nuts})`],["Pravna oblika",mc.legalForm],["Ustanovljeno",`${mc.founded} — ${mc.age} let`],["SKD (glavna)",`${mc.skdMain} — ${mc.skdMainLabel}`]].map(([l,v])=>(<div key={l} style={{display:"flex",alignItems:"baseline",gap:8,padding:"9px 0",borderBottom:`1px solid ${c.border}20`,fontSize:13}}><span style={{color:c.t3,minWidth:100}}>{l}</span><span style={{color:c.t1,fontWeight:500,flex:1}}>{v}</span></div>))}</div><div style={{background:c.white,border:`1px solid ${c.olive}30`,borderLeft:`4px solid ${c.olive}`,borderRadius:16,padding:"20px 22px",marginBottom:24}}><div style={{fontSize:11,fontWeight:700,color:c.olive,marginBottom:14}}>DE MINIMIS STANJE</div><div style={{display:"flex",gap:12,marginBottom:16}}>{[["Prejeto",`${(mc.dm.received/1000).toFixed(0)}.000 €`,c.t1],["Meja","300.000 €",c.t2],["Prosto",`${(mc.dm.free/1000).toFixed(0)}.000 €`,c.olive]].map(([l,v,col])=>(<div key={l} style={{flex:1,textAlign:"center",padding:"12px 8px",background:c.oliveLight,borderRadius:10}}><div style={{fontSize:11,color:c.t3,marginBottom:4}}>{l}</div><div style={{fontSize:17,fontWeight:700,color:col}}>{v}</div></div>))}</div>{mc.dmH.map((h,i)=>(<div key={i} style={{display:"flex",gap:8,fontSize:12,padding:"6px 0"}}><span style={{color:c.t3,minWidth:36}}>{h.y}</span><span style={{color:c.t2,flex:1}}>{h.s}</span><span style={{fontWeight:600,color:c.t1}}>{h.a.toLocaleString("sl-SI")} €</span></div>))}</div><button onClick={()=>setStep(4)} style={btn}>Podatki so pravilni <ArrowRight size={18}/></button></>}
    ...
    {step===6&&<><div style={{textAlign:"center",marginBottom:28}}><div style={{width:64,height:64,borderRadius:20,background:c.olive,display:"inline-flex",alignItems:"center",justifyContent:"center",marginBottom:16}}><Sparkles size={28} color={c.white}/></div><h2 style={{fontSize:24,fontWeight:700,color:c.t1}}>Profil pripravljen</h2></div><div style={{display:"flex",alignItems:"center",gap:16,padding:"20px 22px",borderRadius:16,background:c.oliveLight,border:`1px solid ${c.olive}20`,marginBottom:20}}><div style={{fontSize:36,fontWeight:800,color:c.olive}}>7</div><div><div style={{fontSize:16,fontWeight:600,color:c.t1}}>priložnosti</div><div style={{fontSize:13,color:c.t2}}>3 z ujemanjem nad 80 %</div></div></div><button onClick={onComplete} style={{...btn,height:56,background:c.olive,fontSize:16,fontWeight:700}}>Odpri priložnosti <ArrowRight size={20}/></button></>}
  </div></div>);
}
...
function Dashboard(){
  const [sel,setSel]=useState(grants[0]);const [af,setAf]=useState("Vse");const [showD,setShowD]=useState(true);const [lt,setLt]=useState(true);
  const nav=[{icon:LayoutGrid,label:"Pregled",active:true},{icon:FileText,label:"Razpisi"},{icon:Sparkles,label:"Priložnosti zame",badge:7},{icon:User,label:"Moj profil"},{icon:Bell,label:"Opozorila"},{icon:Calendar,label:"Koledar rokov"},{icon:Bot,label:"AI pomočnik"}];
  return(<div style={{display:"flex",height:"100vh",width:"100%",fontFamily:f,background:c.ivory,color:c.t1,overflow:"hidden"}}>
    <aside style={{width:250,minWidth:250,background:c.graphite,display:"flex",flexDirection:"column",padding:"28px 16px 20px",justifyContent:"space-between"}}><div><div style={{display:"flex",alignItems:"center",gap:12,paddingLeft:12,marginBottom:8}}><div style={{width:38,height:38,borderRadius:10,background:c.olive,display:"flex",alignItems:"center",justifyContent:"center",fontWeight:800,fontSize:14,color:c.white}}>AI</div><div><div style={{color:c.white,fontWeight:700,fontSize:16}}>RAZPISI</div><div style={{color:`${c.white}80`,fontSize:11}}>Pametno do sredstev</div></div></div><nav style={{marginTop:32,display:"flex",flexDirection:"column",gap:4}}>{nav.map(n=><div key={n.label} style={{display:"flex",alignItems:"center",gap:12,padding:"11px 14px",borderRadius:14,cursor:"pointer",background:n.active?c.olive:"transparent"}}><n.icon size={19} strokeWidth={1.75} color={n.active?c.white:`${c.white}85`}/><span style={{fontSize:14,fontWeight:n.active?600:450,color:n.active?c.white:`${c.white}85`,flex:1}}>{n.label}</span>{n.badge&&<span style={{background:n.active?c.white:c.olive,color:n.active?c.olive:c.white,fontSize:11,fontWeight:700,borderRadius:8,padding:"2px 8px"}}>{n.badge}</span>}</div>)}</nav></div><div style={{display:"flex",alignItems:"center",gap:10,padding:"10px 12px"}}><div style={{width:34,height:34,borderRadius:10,background:c.olive,display:"flex",alignItems:"center",justifyContent:"center",fontWeight:700,fontSize:14,color:c.white}}>M</div><div style={{flex:1}}><div style={{color:c.white,fontSize:13,fontWeight:600}}>Mizarstvo d.o.o.</div><div style={{color:`${c.white}55`,fontSize:11}}>Moj profil</div></div></div></aside>
    <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden"}}>
      <header style={{display:"flex",alignItems:"center",gap:16,padding:"16px 28px",background:c.white,borderBottom:`1px solid ${c.border}`}}><div style={{flex:1,display:"flex",alignItems:"center",gap:10,background:c.ivory,border:`1px solid ${c.border}`,borderRadius:12,padding:"12px 16px",height:48}}><Search size={18} color={c.t3}/><input placeholder="Išči po razpisih …" style={{border:"none",background:"transparent",outline:"none",fontSize:14,color:c.t1,fontFamily:f,flex:1}}/></div><div style={{display:"flex",alignItems:"center",gap:10,fontSize:13,color:c.t2}}>Pogovorni jezik<div onClick={()=>setLt(!lt)} style={{width:44,height:24,borderRadius:12,background:lt?c.olive:c.border,position:"relative",cursor:"pointer"}}><div style={{width:18,height:18,borderRadius:"50%",background:c.white,position:"absolute",top:3,left:lt?23:3,transition:"left .2s"}}/></div></div><div style={{position:"relative"}}><Bell size={20} color={c.t2}/><span style={{position:"absolute",top:-4,right:-4,width:16,height:16,borderRadius:"50%",background:c.olive,color:c.white,fontSize:9,fontWeight:700,display:"flex",alignItems:"center",justifyContent:"center"}}>3</span></div></header>
...
export default function App(){
  ...
  if(mode==="onboarding") return <Onboarding onComplete={()=>go("dashboard")} onBack={()=>go("landing")}/>;
  return <Dashboard/>;
}
