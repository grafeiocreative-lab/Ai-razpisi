...
function Onboarding({onComplete,onBack}){
  ...
  useEffect(()=>{if(step!==2)return;setLc([]);setAjpesResult(null);setJodpResult(null);[0,1,2,3].forEach(i=>{setTimeout(()=>setLc(p=>[...p,i]),600+i*700);});let api=false,timer=false;const go=()=>{if(api&&timer)setStep(3);};setTimeout(()=>{timer=true;go();},600+4*700+600);Promise.allSettled([sb.functions.invoke("fetch-ajpes",{body:{registration_number:iv}}),sb.functions.invoke("fetch-jodp",{body:{registration_number:iv}})]).then(([ajpes,jodp])=>{if(ajpes.status==="fulfilled")setAjpesResult(ajpes.value.data);if(jodp.status==="fulfilled")setJodpResult(jodp.value.data);api=true;go();}).catch(()=>{api=true;go();});},[step]);
...
function CompanyProfile({maticna}){
  ...
  const mc={name:company?.company_name||"Podjetje ni najdeno",maticna:company?.registration_number||maticna||"—",address:company?.address||"—",region:company?.region||company?.municipality||"—",nuts:company?.nuts||"—",legalForm:company?.legal_form||"—",founded:company?.founded_year||"—",skdMain:company?.main_activity_code||"—",skdMainLabel:company?.main_activity_name||"—"};
  ...
  if(loading)return(<div style={{padding:"48px",fontFamily:f,color:c.t2}}>Nalagam podatke podjetja …</div>);
  return(<div style={{padding:"28px 28px 60px"}}>
    ...
    <div style={card}>
      ...
      <div>{[["Naslov",mc.address],["Regija",mc.nuts&&mc.nuts!=="—"?`${mc.region} (${mc.nuts})`:mc.region],["Pravna oblika",mc.legalForm],["Ustanovljeno",String(mc.founded)],["SKD",`${mc.skdMain} — ${mc.skdMainLabel}`]].map(([l,v])=>(
